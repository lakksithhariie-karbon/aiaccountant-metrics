-- Slim Mixpanel blobs on public.events. Dashboards only need companyName.
-- Run as postgres (Supabase SQL editor or `supabase db query --linked`).
-- VACUUM FULL cannot run inside a transaction. Run it as its own statement.
...
SET properties = CASE
  WHEN NULLIF(btrim(properties->>'companyName'), '') IS NULL THEN '{}'::jsonb
  ELSE jsonb_build_object('companyName', properties->>'companyName')
END
WHERE properties IS DISTINCT FROM (
  CASE
    WHEN NULLIF(btrim(properties->>'companyName'), '') IS NULL THEN '{}'::jsonb
    ELSE jsonb_build_object('companyName', properties->>'companyName')
  END
