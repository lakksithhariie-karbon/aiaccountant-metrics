...
import { heatmap } from "@/lib/metrics-server";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return jsonOk(await heatmap(bounds));
  } catch (error) {
    const message = error instanceof Error ? error.message : "heatmap unavailable";
    if (message.includes("from_week") || message.includes("to_week") || message.includes("YYYY-MM-DD")) {
      return jsonError(message, 400);
    }
    return jsonError("heatmap unavailable", 500);
  }
}
